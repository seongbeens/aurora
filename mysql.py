...
def createAccount(chat_id):
  try:
    conn = connect()
    cur = conn.cursor()
    # if cur.execute("SELECT * FROM Accounts WHERE telegram_id = '{}'".format(str(chat_id))) == 0:
    cur.execute(
      "INSERT INTO Accounts SET telegram_id = '{}' ".format(chat_id)\
    + "ON DUPLICATE KEY UPDATE date_recreate = current_timestamp")
    conn.commit()
    return True
...
def modifyAccount(chat_id, columns, tuples):
  try:
    ...
    for i, j in zip(columns, tuples):
      cur.execute(
        "INSERT INTO Accounts SET telegram_id = '{}', {} = '{}' ".format(chat_id, i, j)\
      + "ON DUPLICATE KEY UPDATE {} = '{}'".format(i, j))
    conn.commit()
    return True
...
def deleteAccount(chat_id):
  ...
  try:
    ...
    + "WHERE telegram_id = '{}'".format(chat_id))
    conn.commit()
    return True
...
def createVehicle(chat_id, veh_id, veh_name, vin):
  try:
    ...
    + "telegram_id = '{}', vehicle_id = '{}', ".format(chat_id, veh_id)\
    + "vehicle_name = '{}', vin = '{}' ".format(veh_name, vin)\
    + "ON DUPLICATE KEY UPDATE vehicle_name = '{}', vin = '{}' ".format(veh_name, vin))
    conn.commit()
    return True
...
def modifyVehicle(chat_id, veh_id, columns, tuples):
  try:
    ...
    for i, j in zip(columns, tuples):
      cur.execute(
        "INSERT INTO Vehicles SET "\
      + "telegram_id = '{}', vehicle_id = '{}', {} = '{}' ".format(chat_id, veh_id, i, j)\
      + "ON DUPLICATE KEY UPDATE {} = '{}'".format(i, j))
    conn.commit()
    return True
...
def deleteVehicle(chat_id, veh_id = None):
  try:
    ...
    if veh_id:
      ...
      + "WHERE telegram_id = '{}' AND vehicle_id = '{}'".format(chat_id, veh_id))
    else:
      cur.execute(
        "DELETE FROM Vehicles "\
      + "WHERE telegram_id = '{}'".format(chat_id))
